...
export default function PMHub() {
  ...
  function initStorage() {
    try {
      const l = localStorage.getItem("pmhub-log");
      const parsed = l ? JSON.parse(l) : [];
      setLog(parsed);
      const s = localStorage.getItem("pmhub-session");
      if (s && parsed.length >= 3) {
        const days = (Date.now() - Number(s)) / 86400000;
        if (days >= 7) setReviewBanner(true);
      }
      localStorage.setItem("pmhub-session", String(Date.now()));
    } catch {}
  }
  ...
  function addToLog(q) {
    ...
    setLog(prev => {
      ...
      try { localStorage.setItem("pmhub-log", JSON.stringify(next)); } catch {}
      return next;
    });
  }
